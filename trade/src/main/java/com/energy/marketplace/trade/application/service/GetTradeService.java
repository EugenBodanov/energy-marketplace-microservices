...
public class GetTradeService implements GetTradeUseCase {
    ...
    public GetTradeService(LoadTradePort loadTradePort) {
        this.loadTradePort = loadTradePort;
    }

    @Override
    public GetTradeResult getTrade(GetTradeCommand command) {
        Trade trade = loadTradePort.load(command.tradeId());
        return new GetTradeResult(trade.getId(), trade.getBuyerId(), trade.getSellerId(), trade.getListingId(), trade.getAmount());
    }
}
