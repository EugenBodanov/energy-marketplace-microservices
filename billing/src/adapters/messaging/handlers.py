...
def _parse_money(data: dict) -> Money:
    """
    Parses Money from JSON.
    Java sends: {"amount": "100.00", "currency": "EUR"}
    """
    return Money(
...
async def handle_authorize_payment(
    ...
    try:
        ...
        logger.warning(f"[AuthorizePayment] Insufficient funds: {e}")
        await publisher.payment_authorization_failed(trade_id=trade_id, reason=str(e))
    except Exception as e:
        logger.error(f"[AuthorizePayment] Unexpected error: {e}", exc_info=True)
        await publisher.payment_authorization_failed(trade_id=trade_id, reason="Internal error")
        raise
...
async def handle_settle_payment(
    ...
    try:
        ...
        await publisher.payment_settlement_failed(trade_id=trade_id, reason=str(e))
    except Exception as e:
